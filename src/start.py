...
def config_table() -> Panel:
    ...
    table.add_row(" IS_POST_TO_STORY ", ' [green]1=On[/green] ; [red]0=Off[/red] Switch to turn On or Off Reels Auto post to Story ')
    table.add_row(" FETCH_LIMIT ", " Scraper fetch limit in number Ex. 50")
    ...
    table.add_row(" ACCOUNTS ", " Give list of accounts from which you want to scrape Ex. carrayminati,totalgaming_official comma separated ")
    table.add_row(" HASTAGS ", " Enter hashtags which you want to add while auto posting ")
    table.add_row(" LIKE_AND_VIEW_COUNTS_DISABLED ", ' [red]1=Disabled[/red] ; [green]0=Enabled[/green] Switch to turn On or Off Like and View counts ' )
...
if setup == 'y' :
    ...
    if mainConfig.IS_ENABLED_REELS_SCRAPER == "1" :
        ...
        while True:
            mainConfig.FETCH_LIMIT = input("  (FETCH_LIMIT) Define the number of latest reels to be scraped from each account :")
        ...
        while True:
            mainConfig.SCRAPER_INTERVAL_IN_MIN = input("  (SCRAPER_INTERVAL_IN_MIN) Define scraper interval in minutes :")
    ...
    while True:
        mainConfig.IS_POST_TO_STORY = input("  (IS_POST_TO_STORY) Turn On post reels into story? 1=On;0=Off :")
    ...
    if mainConfig.IS_ENABLED_AUTO_POSTER == "1"  or mainConfig.IS_ENABLED_REELS_SCRAPER == "1":
        ...
        mainConfig.ACCOUNTS = input("  (ACCOUNTS) Enter list of usernames from which reels are to be scraped (comma separated) :")
        ...
        while True:
            mainConfig.LIKE_AND_VIEW_COUNTS_DISABLED = input("  (LIKE_AND_VIEW_COUNTS_DISABLED) Enter 1 to Disable or 0 to Enable like and view counts :")
